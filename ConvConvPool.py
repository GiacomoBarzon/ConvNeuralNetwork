...
#CONV+CONV+POOL BLOCKS
#input: number of blocks
def ConvConvPool(blocks_number,
              conv_size=64, pool_size=2,
              image_shape=128, epochs=40, batch_size=64,
              lr=0.0001, decay = True):
    
    print("Blocks Conv+Conv+Pool: N Blocks "+str(blocks_number)+", ConvSize "+str(conv_size)+", PoolSize "+str(pool_size)+"\n\n")
    log_text=open(log_path, "a")
    log_text.write("Blocks Conv+Conv+Pool: N Blocks "+str(blocks_number)+", ConvSize "+str(conv_size)+", PoolSize "+str(pool_size)+"\n\n")
    log_text.close()
    ...
    for i in range(blocks_number-1):
        model.add(Conv2D(conv_size, (3, 3), activation='relu'))
        model.add(Conv2D(conv_size, (3, 3), activation='relu'))
        model.add(MaxPooling2D(pool_size=(pool_size, pool_size)))
    ...
    model.save_weights(str(path_weights)+'weights-ConvConvPool-NBlocks'+str(blocks_number)+'-ConvSize'+str(conv_size)+'-PoolSize'+str(pool_size)+'.h5')  # always save your weights after training or during training
    ...
    plt.savefig(str(path_out)+'loss_curves-ConvConvPool-NBlocks'+str(blocks_number)+'-ConvSize'+str(conv_size)+'-PoolSize'+str(pool_size)+'.png')
    ...
    plt.savefig(str(path_out)+'accuracy_curves-ConvConvPool-NBlocks'+str(blocks_number)+'-ConvSize'+str(conv_size)+'-PoolSize'+str(pool_size)+'.png')
